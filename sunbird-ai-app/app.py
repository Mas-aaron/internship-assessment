...
# ---------------------------------------------------------------------------
# Page config — MUST be the very first Streamlit call, at module level
# ---------------------------------------------------------------------------
...
def main():
    ...
    with col_logo:
        if os.path.exists(_KASUKU_LOGO):
            ...
            st.markdown("🐦")
    with col_title:
        st.markdown("<p class='kasuku-title'>Kasuku</p>", unsafe_allow_html=True)

    st.markdown("Summarise and translate text or audio into a Ugandan local language.")
    st.divider()
